...
| 변수 | 분리 대상 |
| --- | --- |
| `COMPOSE_PROJECT_NAME` | Compose 서비스·네트워크와 기본 이름의 DB·보안 로그·스캐너 볼륨 |
| `OPERIX_PORT` | 호스트 웹 포트. 해당 환경의 `APP_URL`과 프록시 주소도 맞춘다 |
| `OPERIX_IMAGE_TAG` | 앱 이미지 태그 |
| `OPERIX_UPLOAD_VOLUME` | 업로드 볼륨의 명시적 이름. 기본값은 `operix_uploads` |

네 값은 각 환경의 `.env`에 두고, Compose 및 백업·복구 명령을 그 저장소 루트에서 실행한다. 다른 셸 환경변수나 `--project-name`으로 대상이 바뀌지 않는지 확인하고 환경별 백업 경로도 분리한다. 복구 스크립트는 새 업로드 볼륨 이름을 `.env`에 기록하므로, 호출 셸에 `OPERIX_UPLOAD_VOLUME`을 별도로 설정해 이 값을 덮어쓰지 않는다. 검증용 예외 설정을 운영 `.env`에 복사하지 않는다.
...
운영 백업은 age 수신자 공개키를 필수로 받는다. 대상 환경의 저장소 루트에서 실행하며, `.env`와 Compose 프로젝트가 백업할 서비스와 일치해야 한다. 복호화 개인키는 웹 호스트와 분리해 보호하고 복구 담당자가 접근 가능한지 시험한다. 백업에는 자격증명이 들어 있는 runtime.env가 암호화된 아카이브 안에 포함된다. 평문 임시 공간은 접근을 제한하고 종료 시 정리하지만 디스크 보안 삭제를 보장하지 않으므로 호스트 저장 장치 보호도 필요하다.
...
동일 소스/스키마의 애플리케이션 이미지와 `.env`, 실행 가능한 DB를 먼저 준비하고 복구 대상 저장소 루트에서 실행한다. 백업의 `source-commit.txt`와 `runtime.env`는 참고 자료이며 스크립트가 소스를 checkout하거나 설정을 자동 복원하지 않는다. 준비한 이미지·DB·Compose 프로젝트·보안 로그 볼륨이 복구 대상인지 먼저 확인한다. 완전히 잃은 DB 서버라면 빈 DB를 기동하고, **현재까지의 파기 원장과 보안 로그 볼륨을 외부 보관에서 먼저 복구**한다. 오래된 백업에 포함된 파기 기록만 사용하면 그 이후 삭제 요청을 알 수 없다.
